...
void main() {
    ...
    vertex_output.pos = pos.xyz;
    vertex_output.normal = normal;
    vertex_output.view_pos = view_pos.xyz;
    vertex_output.view_normal = view_normal;
    vertex_output.uv = a_Uv;
...
void main() {
    ...
    vec3 albedo_grass = texture(texture_albedo_grass, frag_input.uv).rgb;
    vec3 ground_albedo = mix(albedo_sand, albedo_grass, smoothstep(10, 15, frag_input.pos.y));

    vec2 cliff_uv_x = vec2(frag_input.pos.x, frag_input.pos.y) / 100.0;
    vec2 cliff_uv_z = vec2(frag_input.pos.z, frag_input.pos.y) / 100.0;
